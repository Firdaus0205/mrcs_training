...
$kategori_latihan = "Basic First Aid & CPR + AED";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['markah'])) {
    foreach ($_POST['markah'] as $peserta_id => $mark) {
        ...
        if (mysqli_num_rows($cek) > 0) {
            mysqli_query($conn, "UPDATE keputusan_latihan SET basic = $markah WHERE peserta_id = $peserta_id");
        } else {
            mysqli_query($conn, "INSERT INTO keputusan_latihan (peserta_id, basic) VALUES ($peserta_id, $markah)");
        }
    }
    header("Location: update_result_basic.php?saved=1");
    exit();
}

// Dapatkan peserta dan markah bersama keputusan
$sql = "
SELECT p.id, p.nama, p.no_ic, p.kategori, p.bulan, k.basic AS markah
FROM peserta p
...
<?php else: ?>
<form method="POST" action="update_result_basic.php">
    <table>
        <thead>
            <tr>
                <th>Nama Peserta</th>
